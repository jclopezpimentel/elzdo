<?php if ( ! defined('BASEPATH')) exit('No se permite el acceso directo al script');
 class Mylog{ 
	...
	public function __construct() { 
			$this->path     = "C:/xampp/htdocs/enloszapatosdelotro/log/"; 
			$this->filename = "log"; 
		...
		public function insert($text, $dated, $clear, $backup) { 
			if ($dated) { 
				...
				if ($backup) { 
					$result = (copy($this->path . $this->filename . ".log", 
					$this->path . $this->filename . "_" . str_replace(" ", "_", $this->date) . "-backup.log")) ? 1 : 0; 
